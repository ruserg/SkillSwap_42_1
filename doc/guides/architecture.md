...
**Почему выбран Feature-Sliced Design (FSD):**

1. **Масштабируемость** - FSD четко разделяет уровни абстракции, что упрощает добавление новых фич и рефакторинг
2. **Изоляция** - каждый слой имеет четкие правила импортов, что предотвращает циклические зависимости
3. **Понятность** - структура проекта интуитивна, новый разработчик быстро понимает где что находится
4. **Переиспользование** - shared слой обеспечивает централизованное место для переиспользуемого кода
5. **Тестируемость** - изолированные слои проще тестировать независимо

**Альтернативные подходы и почему они не подошли:**

- **MVC/MVP** - устаревшие паттерны для React приложений, не соответствуют компонентной модели
- **Atomic Design** - фокусируется только на компонентах, не решает вопросы структуры бизнес-логики
- **Произвольная структура** - приводит к хаосу при росте проекта, сложно поддерживать
